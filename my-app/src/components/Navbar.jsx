...
export const Navbar = () => {
  ...
  useEffect(() => {
    if (isAuth) {
      axios
        .get(`https://shopkart-payload.onrender.com/userdata/${isAuth}`)
        .then((res) => {
  ...
  return (
    ...
    <Box style={{ width: "100%" }}>
      <Flex id="topbar" justifyContent="space-between" alignItems="center">
      ...
      <Flex
        ...
        margin="0px"
      >
        <Image src="https://uploads-ssl.webflow.com/63e857eaeaf853471d5335ff/63e86ab4c21faa7bc0bd90dd_Logo.svg" />
        <Flex
          className="hide"
          justifyContent="space-between"
          alignItems="center"
          ...
          {searchdata.length !== 0 && (
            <Box className="search">
              <Flex justifyContent="right">
                <CloseIcon cursor="pointer" onClick={erase} fontSize="xs" />
              </Flex>
              {searchdata.map((e) => (
                <Link
                ...
                >
                  <Flex
                    ...
                    alignItems="center"
                  >
                    <Image width="30px" src={e.image} />
                    <Box textAlign="left">
                      <Text>{e.title}</Text>
                      <Text fontWeight="bold" color="rgb(0, 61, 41)">
                        in {e.category}
                      </Text>
                    </Box>
                  </Flex>
